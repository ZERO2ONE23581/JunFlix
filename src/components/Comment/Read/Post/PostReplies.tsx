...
import { PostCommentInfo } from './CommentInfo';

interface IRepliesProps extends IPostComment {
  comment_id: number;
}
export const PostReplies = ({ post, comment_id }: IRepliesProps) => {
  ...
  return (
    <Cont>
      {data?.replies?.map((reply) => (
        <PostCommentInfo key={reply.id} post={post!} commentId={reply.id} />
      ))}
    </Cont>
  );
};
